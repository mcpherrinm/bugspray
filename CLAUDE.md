...
- **`csr.js`** is a standalone pure-crypto library (like `jws.js`, `SubtleCrypto` is an explicit parameter) that generates an EC P-256 key + PKCS#10 CSR from ACME identifiers. `generateKeyAndCsr` returns the key plus the CSR as DER/base64url/PEM and the private key as PKCS#8 PEM; `buildCsrDer` is the lower-level "CSR from an existing key" entry point. It hand-rolls the minimal ASN.1 DER (SAN extension for dNSName/iPAddress, raw→DER ECDSA signature conversion) but defers the SubjectPublicKeyInfo and private-key encoding to WebCrypto's `exportKey`. The finalize UI (`runFinalizeOrder`) and the CLI (`finalize --gen-csr`) both call it for users who don't already have a CSR; the generated key is surfaced for the user to save, not persisted.
